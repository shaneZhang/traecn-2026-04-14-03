...
from ..utils.exceptions import DataAnalysisError
...
class AnalysisService:
    ...
    def get_descriptive_stats(self, data: pd.DataFrame, column: str) -> Dict[str, float]:
        try:
            ...
            raise DataAnalysisError(f"描述性统计失败: {str(e)}")
    ...
    def get_grouped_stats(self, data: pd.DataFrame, group_by: str, value_col: str) -> pd.DataFrame:
        try:
            if group_by not in data.columns or value_col not in data.columns:
                raise DataAnalysisError(f"列不存在: {group_by} 或 {value_col}")
            ...
            raise DataAnalysisError(f"分组统计失败: {str(e)}")
    ...
    def get_crosstab(self, data: pd.DataFrame, row_col: str, col_col: str,
        ...
        try:
            if row_col not in data.columns or col_col not in data.columns:
                raise DataAnalysisError(f"列不存在: {row_col} 或 {col_col}")
    ...
    def compare_by_dimension(self, data: pd.DataFrame, dimension: str,
        ...
        try:
            if dimension not in data.columns:
                raise DataAnalysisError(f"维度列 '{dimension}' 不存在")
            ...
            raise DataAnalysisError(f"维度对比失败: {str(e)}")
    ...
    def get_trend_analysis(self, data: pd.DataFrame, time_col: str, value_col: str) -> pd.DataFrame:
        try:
            ...
            if not pd.api.types.is_numeric_dtype(data[value_col]):
                raise DataAnalysisError(f"值列必须是数值类型")
    ...
    def get_percentile_distribution(self, data: pd.DataFrame, column: str,
        ...
        try:
            ...
            self.logger.error(f"百分位分布计算失败: {str(e)}")
            raise DataAnalysisError(f"百分位分布计算失败: {str(e)}")
    ...
    def get_salary_distribution(self, data: pd.DataFrame, salary_col: str = 'pre_tax_salary',
        ...
        try:
            ...
            self.logger.error(f"薪资分布计算失败: {str(e)}")
    ...
    def get_summary_report(self, data: pd.DataFrame, salary_col: str = 'pre_tax_salary') -> Dict[str, Any]:
        try:
            ...
            raise DataAnalysisError(f"汇总报告生成失败: {str(e)}")
    ...
    def get_boxplot_data(self, data: pd.DataFrame, group_by: str, value_col: str) -> Dict[str, Dict]:
        try:
            ...
            self.logger.error(f"箱线图数据生成失败: {str(e)}")
            raise DataAnalysisError(f"箱线图数据生成失败: {str(e)}")
